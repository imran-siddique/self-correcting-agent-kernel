...
% -- NEURIPS / ICML FORMATTING PACKAGES --
% (Using standard article for generic arXiv compatibility, 
%  replace with \usepackage{neurips_2026} for actual submission)
...
% Page geometry for arXiv (approx. standard academic margins)
\geometry{
 a4paper,
 total={170mm,257mm},
 left=20mm,
...
While SCAK focuses on the \textit{internal} alignment and capability restoration of the agent (Loop 2), the enforcement of strict safety boundaries and access control is handled by the \textbf{Agent Control Plane}. We refer readers to our companion paper [Anonymous, 2026] for details on the runtime governance mechanisms (Loop 1) that complement SCAK's self-correction.
...
\begin{enumerate}
    ...
    \item \textbf{Secure Dual-Loop OODA:} Addressing recent critiques of OODA loops as security vulnerabilities [Schneier et al., 2026], we decouple the \textbf{Runtime Loop} (fast, safety-constrained) from the \textbf{Alignment Loop} (slow, teacher-verified). This ensures that untrusted user inputs cannot permanently corrupt the agent's core instructions without rigorous, asynchronous verification.
\end{enumerate}

\section{Related Work}

\subsection{Self-Correcting Language Agents}
Recent work has explored using LLMs to correct their own output. \textbf{Reflexion} [Shinn et al., 2023] pioneered verbal reinforcement learning, where agents maintain a text-based trace of past failures. While effective for short-horizon tasks, Reflexion lacks a memory lifecycle; reflection traces accumulate indefinitely, eventually saturating the context window. \textbf{Self-Refine} [Madaan et al., 2023] demonstrates iterative improvement without external rewards but relies on the model's own capability to critique itself. Our ablation studies ($\S$4.5) show that self-critique fails on ``laziness'' tasks because the model shares the same blind spots as its generator. SCAK overcomes this by using a stronger external teacher (o1-preview) and introducing \textit{Semantic Purge} to prevent the ``technical debt of belief.''
...
\begin{tikzpicture}
    ...
    % Note: Arrow direction usually implies flow, let's fix semantics
...
\subsection{Multi-Agent Systems and Skill Libraries}
\textbf{Voyager} [Wang et al., 2023] introduced the concept of an open-ended skill library for embodied agents (Minecraft). While Voyager demonstrates the power of persistent skills, its library grows monotonically. In enterprise settings, monotonic growth leads to \textit{Context Rot}. SCAK's contribution is the \textbf{Type A/B Taxonomy}, which provides a principled mechanism for \textit{forgetting} skills that become obsolete due to foundational model improvements (e.g., GPT-4o $\rightarrow$ GPT-5).

\subsection{AI Safety and Governance}
Frameworks like \textbf{Constitutional AI} [Bai et al., 2022] and \textbf{NeMo Guardrails} focus on alignment with static safety principles. While necessary, these systems do not address \textit{capability} failures (laziness). An agent can be perfectly aligned (harmless) but entirely useless (lazy). SCAK complements these frameworks by targeting the \textit{quality} dimension of alignment, using \textbf{Differential Auditing} to distinguish between genuine constraints and artificial laziness.
...
SCAK implements the OODA loop as two concurrent processes:
\begin{itemize}
...
To prevent Context Rot, we classify every patch $P$ into a Decay Category $D(P)$:
\begin{itemize}
...
We extended the GAIA benchmark [Mialon et al., 2023] with 50 ``vague queries'' designed to trigger laziness (e.g., vague search parameters where data exists).
...
\begin{itemize}
    ...
    \item \textbf{Cold Start:} New agents require a ``training period'' (approx. 7 days) to populate their semantic memory, during which performance trails mature agents.
\end{itemize}

\section{Conclusion}We presented the \textbf{Self-Correcting Agent Kernel (SCAK)}, a system that challenges the prevailing dogma of ``more context is better.'' By coupling \textbf{Differential Auditing} (which detects missing capabilities at 10% the cost of full verification) with \textbf{Semantic Purge} (which actively removes obsolete instructions), we achieve a system that improves with age rather than degrading under the weight of its own prompt.Our empirical results on the GAIA extension demonstrate that reliability in agentic systems stems not from raw model intelligence, but from hygienic memory management. The capability to achieve a $<30$s MTTR in chaos scenarios suggests that SCAK is a viable kernel for mission-critical enterprise deployments.Ultimately, SCAK shifts the paradigm from \textit{Scale by Addition} to \textbf{Scale by Subtraction}. Future work will focus on federating competence patches across disparate agent fleets and exploring self-distillation techniques to remove the dependency on external teacher models entirely. This work serves as the foundational \textit{alignment engine} for autonomous systems; for the complementary \textit{runtime governance} architecture, we refer readers to our concurrent work on the Agent Control Plane [Cite ACP].
...
All experiments were run on standard cloud CPU instances (Azure Standard\_D4s\_v3). No GPU acceleration was required as the primary workload consists of API calls to OpenAI models. Total compute cost for the full benchmark suite is $<\$10$.
